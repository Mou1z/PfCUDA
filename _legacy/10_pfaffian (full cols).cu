...
__global__ void rowUpdate(
    ...
    const int c = k + 1 + blockIdx.x;

    double value = 0.0;
    const int stride = blockDim.x * 2;
    for(int r = (tid * 2); r < rows - 1; r += stride) {
        const double Ak0 = CM(A, n, r, k);
        const double Ak1 = CM(A, n, r + 1, k);

        double a = CM(A, n, r + 1, c) * Ak0;
        double b = CM(A, n, r, c) * Ak1;
        value += (a - b);
    }
    ...
    value = (tid < warps) ? shared[tid] : 0.0;
...
double pfaffian(const double * _A, const long n) {
    ...
    for(int k = 0; k < n - 1; k += 2) {
        ...
        rowUpdate<<<cols, 1024, 32 * sizeof(double)>>>(A, n, k, rows, cols);
        ...
        rowUpdate<<<cols - 1, 1024, 32 * sizeof(double)>>>(A, n, k + 1, rows, cols - 1);
