...
def pivot(data):
    P, i, j = data
    P = P.at[jnp.array([i, j])].set(P[jnp.array([j, i])])
    return P, -1.0
...
def pfaffian(A):
    ...
    def body(data, k):
        ...
        R2 = (A[:, P[k + 1]] - dR(V, W, P[k + 1]))
        W = W.at[:, i].set(R2)

        return (A, V, W, P, result), None

    (_, V, W, P, result), _ = lax.scan(body, (A, V, W, P, 1.0), jnp.arange(0, n, 2))
