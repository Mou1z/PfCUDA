...
# Configuration
# -----------------------------
SIZES = [i for i in range(20, 7000, 500)]
RUNS_PER_SIZE = 5
...
for file in historical_files:
    with open(os.path.join(RESULTS_DIR, file), "r") as f:
        ...
        plt.plot(h_sizes, h_means, color='#bdc3c7', alpha=0.2, linewidth=1, zorder=1)

# 2. Plot Current Run
current_data = sorted([(int(k), v["mean"]) for k, v in current_results["sizes"].items()])
c_sizes, c_means = zip(*current_data)
plt.plot(c_sizes, c_means, marker="o", markersize=6, color="#2980b9", 
         linewidth=2.5, label="Current Run", zorder=5)
